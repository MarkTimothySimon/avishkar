...
def load_predictions(file_path='2025_predictions.csv'):
    try:
        ...
        st.error("Predictions file not found. Please ensure the file is in the same directory.")
        return None
...
if df is not None:
    with st.sidebar:
        ...
        timeline_days = st.slider("Days to display", 7, 60, 30)
        
        st.markdown("---")
        st.markdown("### About CVI")
        st.info("""
